 ...
 * @author Hu Yi-Qi
 * @time 2016.4.24
 * this is a specific optimization implemented in SRE
 */
...
public class RACOS extends Method{
	...
	//SRE parameters
	private Dimension ori_dimension;
	...
	public void setMaxIteration(int bud){
		MaxIteration = bud;
	}
...
//					System.out.println("["+model.region[i][0]+", "+model.region[i][1]+"]");
	...
	protected void Initialize(){
		...
		for(int i=0; i<SampleSize+PositiveNum; i++){
			...
			map_x = MappingBack(temp[i], dimension, ori_dimension, projection, bias);
			temp[i].setValue(task.getValue(map_x) + punishing);
			BudCount++;
		}	
	 ...
	 * 
	 */
	protected void UpdatePosPop(Instance temp){
...
//			Optimal.PrintInstance();
	...
	@Override
	public Instance Optimization(Dimension sub_dim, Dimension ori_dim, Task t, double[][] proj_matrix, double[] b) {
		...
		for(; BudCount<this.Budget; ){	
			...
			while (reSample) {		
				...
				temp = RandomInstance(PosPop[ChoosenPos]);//sample
				if (notExistInstance(0, temp)) {
					map_x = MappingBack(temp, dimension, ori_dimension, projection, bias);
					temp.setValue(task.getValue(map_x) + punishing);//query
					BudCount++;
					reSample = false;
				}
			}
/*			System.out.println("budget:"+BudCount);
			...
			UpdatePosPop(temp);//update PosPop set
